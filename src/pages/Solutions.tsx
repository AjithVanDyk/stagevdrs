...
const Solutions = () => {
	...
	// Solution items with translations - memoized to prevent expensive re-computation
	const solutionItems: Solution[] = useMemo(() => [
		{
			id: 1,
	...
	useEffect(() => {
		...
		if (showSolutionModal || showQuoteForm) {
	...
	}, [showSolutionModal, showQuoteForm]);

	// Focus trap for modals
	useEffect(() => {
		if (showSolutionModal && modalRef.current) {
	...
	useEffect(() => {
		if (showQuoteForm && quoteModalRef.current) {
			const focusableElements = quoteModalRef.current.querySelectorAll(
				'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
			);
			const firstElement = focusableElements[0] as HTMLElement;
			const lastElement = focusableElements[focusableElements.length - 1] as HTMLElement;
			...
			quoteModalRef.current.addEventListener('keydown', handleTab);
			firstElement?.focus();

			return () => {
				quoteModalRef.current?.removeEventListener('keydown', handleTab);
			};
		}
	}, [showQuoteForm]);
					...
					>
						{solutionItems.map((solution) => (
							<motion.div
								...
								className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 cursor-pointer group focus:outline-none focus:ring-2 focus:ring-vd-blue focus:ring-offset-2"
							>
						...
						>
							<div className="p-6">
								<div className="flex justify-between items-center mb-6">
									<h2 id="quote-modal-title" className="text-2xl font-bold text-vd-blue-dark">{t('common.getAQuote')}</h2>
									<button
										onClick={() => setShowQuoteForm(false)}
										className="p-2 hover:bg-gray-100 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-vd-blue"
				...
				{showSolutionModal && selectedSolution && (
					...
					>
						...
						>
							<div className="p-6">
								<div className="flex justify-between items-center mb-6">
									<h2 id="solution-modal-title" className="text-2xl font-bold text-vd-blue-dark">{selectedSolution.name}</h2>
									<button
										onClick={() => setShowSolutionModal(false)}
										className="p-2 hover:bg-gray-100 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-vd-blue"
										aria-label={`Close ${selectedSolution.name} modal`}
									>
										<X className="w-6 h-6" />
									</button>
								</div>
