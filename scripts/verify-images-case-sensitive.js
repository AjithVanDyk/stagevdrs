...
function verifyImages() {
  ...
  if (missing.length > 0) {
    ...
    missing.forEach(path => {
      ...
      const similar = actualFiles.filter(f => {
        ...
        const actualFileName = f.actualPath.split('/').pop()?.toLowerCase() || '';
        return actualFileName.includes(fileName.replace(/\.(jpg|jpeg|png|gif)$/i, '')) || 
               fileName.includes(actualFileName.replace(/\.(jpg|jpeg|png|gif)$/i, ''));
      });
      if (similar.length > 0) {
        console.log(`   💡 Suggested fix - Use one of these files instead:`);
        similar.slice(0, 3).forEach(f => console.log(`      ${path.replace(/[^/]+$/, f.actualPath.split('/').pop() || '')}`));
      }
      console.log('');
    });
  }
  ...
  if (caseMismatches.length > 0) {
    ...
    console.log(`   ${caseMismatches.length} case mismatch(es) detected`);
    console.log('   These will cause build failures on Vercel (case-sensitive filesystem)');
    console.log('   Please fix the case mismatches listed above.\n');
    process.exit(1);
  } else if (missing.length > 0) {
    console.log('=' .repeat(60));
    console.log('⚠️  VERIFICATION WARNING');
    console.log(`   ${missing.length} missing image reference(s) found`);
    console.log('   The build will continue, but these images may not display correctly.');
    console.log('   Please update the file paths listed above.\n');
    process.exit(0);
  } else {
    console.log('=' .repeat(60));
    console.log('✅ VERIFICATION PASSED');
    console.log('   All image references verified successfully\n');
    process.exit(0);
  }
}
